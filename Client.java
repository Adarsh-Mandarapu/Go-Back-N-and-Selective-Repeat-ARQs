...
class Client
{
	...
	public static String transferFilename = "data.txt";
		...
		if (protocol.equals("GBN")) {
			...
			while (waitingForAck <= lastPacketNo) {
				...
				while(nextPacket - waitingForAck < WINDOW_SIZE && !endOfFile){
					...
					PacketClass dataPacket = new PacketClass(sqeNo, partData, endOfFile);
				...
				try{
					...
					AckClass ackPacket = new AckClass(receivePacket.getData());
					Thread.sleep(100);
					System.out.println("");
					...
					for(int i = waitingForAck; i < nextPacket; i++){
						...
						PacketClass dataPacket = new PacketClass(sqeNo, partData, endByte > FILE_SIZE);
			...
			HashMap<Integer, AckClass> receivedAcks = new HashMap<Integer, AckClass>();
			...
			while (waitingForAck <= lastPacketNo) {
				...
				while(nextPacket - waitingForAck < WINDOW_SIZE && !endOfFile){
					...
					PacketClass dataPacket = new PacketClass(nextPacket % lastSeqNo, partData, endOfFile);
				...
				try{
					...
					PacketClass dataPacket = new PacketClass(waitingForAck % lastSeqNo, partData, endByte > FILE_SIZE);
