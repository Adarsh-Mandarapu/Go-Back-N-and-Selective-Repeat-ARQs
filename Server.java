...
class Server
{
	...
	public static void main(String args[]) throws Exception
	{
        ...
        int receivingPortnumber;

        // check if bit length is a valid integer
		try{
			receivingPortnumber = Integer.parseInt(args[0]);
		...
		// Open Socket
		DatagramSocket serverSocket = new DatagramSocket(receivingPortnumber);
		...
		if (protocol.equals("GBN")) {
			...
			while(!end)
			{
				...
				PacketClass packet = new PacketClass(receiveData, receivePacket.getLength());
				...
				AckClass ackPacket = new AckClass((waitingForPacket - 1) % lastSeqNo);
			...
			HashMap<Integer, PacketClass> receivedPackets = new HashMap<Integer, PacketClass>();
			...
			while(!end)
			{
				...
				if (packet.isValidPacket()) {
					...
					while(receivedPackets.containsKey(waitingForPacket)){
						PacketClass bufferedPacket = receivedPackets.get(waitingForPacket);
					...
					// Create and ACK Packet with the sequence number
					AckClass ackPacket = new AckClass(packet.getSeqNo());
					byte[] ackData = ackPacket.generatePacket();
